...
| Apple Business Connect | `https://businessconnect.apple.com/` | Opened 28/05/2026; requires Apple sign-in / owner setup. Public Apple Maps search for `Brasserie du Chateau de Durbuy` resolves to `Durbuy Castle`, not the brewery. | Owner-login setup is a high-priority discovery fix. |
| Bing Places | `https://www.bingplaces.com/` | Opened 28/05/2026; requires Microsoft/Bing sign-in / owner setup. Public Bing Maps search for the official name resolves to `Brasserie de Durbuy srl`, not Brasserie du Château de Durbuy. | Owner-login setup is a high-priority discovery fix. |
...
| Bizique / AllBiz | `https://www.bizique.be/la-ferme-au-ch%C3%AAne` | Old `La Ferme au Chêne` is marked closed, but still lists old address and surfaces `Brasserie Marckloff` as another business at the same address. Bizique exposes `Modifier ou supprimer`, a support ticket form, and a removal tool. | Use edit/remove route if accessible; lower priority because the old record is already marked closed. |
| Nonoresto | `https://nonoresto.be/restaurant/la-ferme-au-chene/` | New stale Dutch restaurant page found 28/05/2026; presents La Ferme au Chene as a current brewery cafe/restaurant. Contact form refilled with a short Dutch correction message in browser, not submitted. | Review visible form and submit if approved. |
...
| Nonoresto | `https://nonoresto.be/contact/` | Refilled 28/05/2026 with a shorter Dutch correction message for the stale `La Ferme au Chene` restaurant page. Not submitted; left visible in the browser for owner review. |
